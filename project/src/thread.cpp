...
namespace dc
{
	CThread::CThread(const unsigned int identifier, CTaskManager* taskManager) :
		m_id(identifier),
	...
	const unsigned int CThread::Identifier() const
	{
		return m_id;
	}
	
	const bool CThread::PendingTasks() const
	{
		return !m_taskQueue.empty() || mp_currentTask;
	}
	
	const bool CThread::WaitCondition()
	{
		return m_finish || PendingTasks();
	}
	
	const unsigned int CThread::NumTasks() const
	{
	...
	void CThread::Run()
	{
		while(!m_finish)
		{
			if(!mp_currentTask)
			{
				...
				std::unique_lock<std::mutex> lock(m_queueMutex);
				m_condition.wait(lock, std::bind(&CThread::WaitCondition, this));
