...
TEST(MTTaskManager, TestLambdaTask)
{
    auto* taskManager = new dc::CTaskManager();
    ...
    auto functionTask1 = [&] {
        ...
        auto functionTask2 = [&] { DotProductAtomic(v1, v2, result, bounds[1], bounds[2]); };
        taskManager->Add(functionTask2, 0);
    };

    auto functionTask3 = [&] { DotProductAtomic(v1, v2, result, bounds[2], bounds[3]); };

    auto functionTask4 = [&] { DotProductAtomic(v1, v2, result, bounds[3], bounds[4]); };
